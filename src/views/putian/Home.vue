<template>
  <div ref="dashboardContainerRef" class="dashboard-container">
    ...
    <mapContainer
      ...
      :road-condition-markers="roadConditionMarkers"
      :traffic-event-markers="trafficEventMarkers"
      @road-condition-marker-click="handleRoadConditionMarkerClick"
...
const isTrafficEventCardEmpty = computed(
  () =>
    hasLoadedTrafficEventCard.value && Number(totalEventsCount.value || 0) === 0
);
...
const buildRoadConditionVideoColumnsParams = async (detailData) => {
  ...
  try {
    const response = await http.get(API_URLS.video.getVideoListByFlags, {
      flagid,
    });
...
const fetchTrafficJamTrackPolylines = async (trafficData, requestId) => {
  ...
  try {
    const routeRaw = await http.get(API_URLS.putian_highway.setPositionByNode, {
...
const fetchExceptionSectionData = async () => {
  ...
  try {
    ...
    const data = await http.get(API_URLS.putian_highway.getTeQingSection);
...
const goToBillingReports = () => {
  // 功能：跳转至收费报表页面路由。
  router.push('/putian/billing-reports');
};
</script>
