<template>
  <div class="call-category-page">
    <div class="toolbar">
      <div class="toolbar-title">数据列表</div>
      <div class="toolbar-actions">
        <label class="month-label" for="categoryMonth">月份:</label>
        <input
          id="categoryMonth"
    ...
    <div class="table-wrap">
      <table class="report-table">
        <thead>
          <tr>
            <th rowspan="2" class="first-col">业务分类</th>
            <th colspan="4">{{ monthTitle }}</th>
            <th colspan="2">同比</th>
            <th colspan="2">环比</th>
          </tr>
          <tr>
        ...
        <tbody>
          <tr v-for="row in tableRows" :key="row.category">
            <td>{{ row.category }}</td>
            <td>{{ row.monthConnected }}</td>
            <td>{{ row.dailyVolume }}</td>
            <td>{{ row.monthRate }}</td>
            <td>{{ row.monthShare }}</td>
            <td>{{ row.yoyConnected }}</td>
            <td>{{ row.yoyRate }}</td>
            <td>{{ row.momDailyConnected }}</td>
            <td>{{ row.momRate }}</td>
          </tr>
        </tbody>
      </table>
