...
export default {
  ...
  methods: {
    ...
    async fetchAndEmitSectionTrackByLinkId(linkId) {
      ...
      try {
        const data = await http.get(API_URLS.putian_highway.setPositionByNode, {
    ...
    async getErrorInfoByPlate({ passid, errorKey, nowTime }) {
      return await http.get(API_URLS.putian_highway.getErrorInfoByPlate, {
        passid,
        errorKey,
        nowTime,
      });
    ...
    async getTeQingList(flagId) {
      ...
      try {
        const data = await http.get(API_URLS.putian_highway.getTeQingList, {
          flagId: flagId,
