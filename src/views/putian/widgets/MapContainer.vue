...
const PUTIAN_GEOJSON_URL = '/350300_full.json';
const PUTIAN_TRACK_CENTER = [118.96, 25.45];
const PUTIAN_TRACK_BOUNDS = {
  minLng: 117.5,
  maxLng: 120.8,
  minLat: 24.0,
  maxLat: 26.2,
...
export default {
  ...
  data() {
    return {
      ...
      // 文本标注默认隐藏
      labelVisible: false,
      clusteredPointTypes: ['camera', 'variableMessageSign'],
  ...
  methods: {
    ...
    async initMap() {
      try {
        ...
        this.jmap = new joint.JMap(container.id || this.mapId, {
          ...
          //       // 查询条件
          //       where:
          //         "gantry_hex in ('34082D','34082F','340831','340833','340835','340B01','340B03','340B05','340B07','340B08','340B09','340B0B','340B0D','341A01','341A03','341A05','341A07','341A09','341A0B','341A0D','341A0F','341A11','341A12','341A13','35082D','35082F','350831','350833','350835','350B01','350B03','350B05','350B07','350B08','350B09','350B0B','350B0D','351A01','351A03','351A05','351A07','351A09','351A0B','351A0D','351A0F','351A11','351A12','351A13')",
          ...
          //       // 查询条件
          //       where:
          //         "name in ('东进服务区', '善乡服务区', '大济服务区', '菜溪岩服务区')",
          //       resultRecordCount: 100000,
          //     },
          //   },
          //   // 可以在这里添加更多图层配置
        ...
        // 加载莆田行政区 GeoJSON 并绘制边界与遮罩
        await this.loadPutianGeojson();
        this.drawPutianBoundaryAndMask();
        this.syncTrackPolylines();
        this.syncRoadConditionMarkers();
        this.syncTrafficEventMarkers();
     ...
     * 加载莆田市行政区 GeoJSON 数据
     */
    async loadPutianGeojson() {
      try {
        const res = await fetch(PUTIAN_GEOJSON_URL);
        if (!res.ok) {
          console.error('加载莆田 GeoJSON 失败:', res.statusText);
          return;
        }
        const geojson = await res.json();
        if (!geojson || !Array.isArray(geojson.features)) {
          console.error('莆田 GeoJSON 数据格式不正确:', geojson);
          return;
        }
        this.putianFeatures = geojson.features;
      } catch (e) {
        console.error('请求莆田 GeoJSON 发生错误:', e);
      }
    },
     ...
     * 绘制莆田边界线，并叠加外部暗色遮罩
     */
    drawPutianBoundaryAndMask() {
      if (!this.jmap || !this.layer2 || !this.putianFeatures.length) {
        return;
      }
      ...
      // 1. 绘制莆田各区县边界线
      // -----------------------------
      const features = this.putianFeatures;
      ...
      features.forEach((feat) => {
        ...
        const handleRing = (ring) => {
          ...
          putianOuterRings.push(normalizedRing);
      ...
      try {
        ...
        const normalizedHoleRings = putianOuterRings.map((ring) =>
    ...
    scoreTrackCoordinateStrategy(points = [], strategy) {
      ...
      sample.forEach((point) => {
        ...
        if (this.isPointInsideBounds(converted, PUTIAN_TRACK_BOUNDS)) {
          putianCount += 1;
        }
        ...
        totalDistancePenalty += Math.hypot(
          lng - PUTIAN_TRACK_CENTER[0],
          lat - PUTIAN_TRACK_CENTER[1]
        );
      });
    ...
    async loadRemoteFeatures() {
      ...
      try {
        const res1 = await fetch(gantryUrl);
        const data1 = await res1.json();
        // console.log(data1);
        this.addFeaturesToLayer(data1, 'gantry');

        // 2. 加载服务区数据
        const serviceUrl =
          'http://35.80.236.142:8888/admin-api/Features/gis_service_area_fj/JointFeature?ak=0620aae950f94394ba7c4164100aa50b&where=' +
          encodeURIComponent(
            "(name LIKE '%善乡%' OR name LIKE '%大济%' OR name LIKE '%菜溪岩%' OR name LIKE '%白沙%' ) AND city LIKE '%莆田%'" // 东进服务区筛除
          ) +
          '&resultRecordCount=100000';
        const res2 = await fetch(serviceUrl);
        const data2 = await res2.json();
        // console.log(data2);
        this.addFeaturesToLayer(data2, 'service');
        // 3. 加载收费站数据
        const tollStationUrl =
          ...
          encodeURIComponent(
            "code in ('3103', '3101', '3102', '4508', '4507', '4509', '4505', '4504', '4501', '3108', '4503', '4502', '3107', '4406', '4405', '4403', '4404', '4401', '4402', '4506')"
          ) +
          '&resultRecordCount=100000';
        const res3 = await fetch(tollStationUrl);
        const data3 = await res3.json();
        // console.log(data2);
        this.addFeaturesToLayer(data3, 'tollStation');

        // 4. 加载摄像头点位数据
        // 旧接口（按要求保留注释）：
        // const cameraUrl =
        //   'http://35.80.236.142:8888/admin-api/Features/gis_monitoring_point_fj/JointFeature?ak=0620aae950f94394ba7c4164100aa50b&where=' +
        //   encodeURIComponent(
        //     "route_no LIKE '%G15%'  AND org_1 LIKE '%莆田%' AND chan_name LIKE '%收费站%' AND chan_name NOT LIKE '%泉港%' AND chan_name NOT LIKE '%惠安%' AND chan_name NOT LIKE '%驿坂%' "
        //   ) +
        //   '&resultRecordCount=100000';
        const cameraUrl =
          'http://35.80.236.142:8888/admin-api/Features/jksxt/JointFeature?ak=0620aae950f94394ba7c4164100aa50b&where=' +
          encodeURIComponent("org_name LIKE '%莆田%'") +
          '&resultRecordCount=100000';
        const res4 = await fetch(cameraUrl);
        const data4 = await res4.json();
        this.addFeaturesToLayer(data4, 'camera');
        ...
        const variableMessageSignUrl =
          ...
          encodeURIComponent(
            'fid IN (1955,1930,1929,1928,1927,1956,1933,1934,1952,1953,1951,1954,1630,1957,1960,1947,1958,1949,1959,1950,1939,1941,1940,1936,1935,1938,1948,1979,1981,1982,1983,1984,1980,1970,1964,1971,1969,1974,1975,1972,1968,1967,1973,1978,1961,1965,1962,1966,1943,1942,1976,1977,1629,1631,1931,1022,1023,1024,1025,1026,1027,1963,1937,1932,1926,1945)'
          ) +
          '&resultRecordCount=100000';
        const res5 = await fetch(variableMessageSignUrl);
        const data5 = await res5.json();
        this.addFeaturesToLayer(data5, 'variableMessageSign');
      } catch (err) {
    ...
    getPointClusterLevelConfigs() {
      return [
        { key: 'l0', maxZoom: 10.8, cellSize: 108, sampleZoom: 10.8 },
        { key: 'l1', maxZoom: 11.6, cellSize: 92, sampleZoom: 11.6 },
        { key: 'l2', maxZoom: 12.4, cellSize: 76, sampleZoom: 12.4 },
        { key: 'l3', maxZoom: 13.2, cellSize: 62, sampleZoom: 13.2 },
        { key: 'l4', maxZoom: 14.0, cellSize: 48, sampleZoom: 14.0 },
        { key: 'l5', maxZoom: 14.8, cellSize: 36, sampleZoom: 14.8 },
        { key: 'l6', maxZoom: 15.6, cellSize: 26, sampleZoom: 15.6 },
        {
          key: 'l7',
    ...
    resolvePointClusterLevel(zoom) {
      const currentZoom = Number(zoom);
      const configs = this.getPointClusterLevelConfigs();
    ...
    buildPointClusterHierarchy(type) {
      ...
      const levelConfigs = this.getPointClusterLevelConfigs();
      const hierarchy = {};
    ...
    renderPointClusterLevel(
      ...
      const levelConfig = this.resolvePointClusterLevel(zoom);
