...
const fetchTrafficThresholds = async (flagIds = []) => {
  ...
  const results = await Promise.allSettled(
    requestFlagIds.map(async (flagId) => {
      ...
      const data = await http.get(API_URLS.putian_highway.getThreadholdInfo, {
...
const fetchTrafficData = async (options = {}) => {
  ...
  try {
    ...
    const countTask = http.get(API_URLS.putian_highway.getPTTopoCount);
    const jamTask = http.get(API_URLS.putian_highway.getJamRoads, {
      nowTime: getNow(),
      eventType: '0,1',
    });
...
onMounted(() => {
  initChart();
  // 初始加载数据（并行获取总数和拥堵列表）
  fetchTrafficData({ showLoading: false });
  // 可以添加定时刷新逻辑
  // setInterval(fetchTrafficData, 30000); // 每30秒刷新一次
});
