...
const loadVideoUrlsForPage = async (page = 1, seq = requestSeq.value) => {
  ...
  await withConcurrency(
    ...
    async (video) => {
      ...
      try {
        const urlResponse = await get(API_URLS.video.getVideoUrl, {
...
const loadVideoList = async () => {
  ...
  try {
    ...
    if (eventVideoParams) {
      const response = await get(API_URLS.video.getVideoList, eventVideoParams);
      if (seq !== requestSeq.value) return;
    ...
    const responses = await Promise.all(
      roadFilters.map((item) =>
        get(API_URLS.video.getVideoList, {
          ...baseParams,
