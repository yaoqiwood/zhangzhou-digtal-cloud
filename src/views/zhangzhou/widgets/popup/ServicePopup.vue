...
export default {
  ...
  methods: {
    ...
    resolveServiceAreaCode(data = {}) {
      ...
      return this.getServiceAreaCode(data?.sub_name, data?.direction);
    },
    // 获取服务区基本信息
    async fetchServiceAreaBaseInfo() {
      try {
        ...
        const serviceAreaCode = this.resolveServiceAreaCode(this.data);

        if (!serviceAreaCode) {
          console.error('无法获取服务区代码', this.data);
          return;
        }
