<template>
  <div class="dashboard-container">
    ...
    <div class="billing-content-bg">
      <div class="billing-content-title">
        <span class="billing-content-title-label">收费报表</span>
        <button
          type="button"
          class="billing-agent-button"
          @click="openBillingReportAgent"
        >
          收费报表智能体
        </button>
      </div>
    ...
    <BillingReportAgentModal v-model="isBillingReportAgentVisible" />
...
import MonthlyTollComparisonReportPage from './reports/MonthlyTollComparisonReportPage.vue';
import BillingReportAgentModal from './widgets/BillingReportAgentModal.vue';
...
const activeTitle = ref('月征费比较表');
const isBillingReportAgentVisible = ref(false);
const billingPanelBodyRef = ref(null);
const panelLoading = ref(false);
let panelLoadingObserver = null;

const currentReportComponent = computed(() => MonthlyTollComparisonReportPage);

const openBillingReportAgent = () => {
  isBillingReportAgentVisible.value = true;
};
...
.billing-content-bg {
  ...
  .billing-agent-button {
    ...
    border-radius: 5px;
    background: rgba(18, 109, 177, 0.5);
    color: #e4fcff;
    font: inherit;
    ...
    &:hover {
      ...
      box-shadow: 0 0 10px rgba(85, 217, 255, 0.45);
    }
